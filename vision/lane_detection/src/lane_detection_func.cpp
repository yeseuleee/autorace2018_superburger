...
//                            *it = 1;
            ...
            void CalLane::detectWhiteRange(cv::Mat src, cv::Mat& dst, int hmin, int hmax, int smin, int smax, int vmin, int vmax, int amin, int amax){
            ...
            void CalLane::birdEyeView_left(cv::Mat src, cv::Mat& dst) {
                ...
                inputQuad[0] = cv::Point2f(50, dst.rows*0.5);
                inputQuad[1] = cv::Point2f(dst.cols-1, dst.rows*0.5);
                inputQuad[2] = cv::Point2f(dst.cols-1, dst.rows-1);
                inputQuad[3] = cv::Point2f(dst.cols/2, dst.rows-20);
                // The 4 points where the mapping is to be done , from top-left in clockwise order
                outputQuad[0] = cv::Point2f(0, 0);
                outputQuad[1] = cv::Point2f(dst.cols-1, 0);
                outputQuad[2] = cv::Point2f(dst.cols*0.4-50, dst.rows - 1);
                outputQuad[3] = cv::Point2f(dst.cols*0.1, dst.rows - 20);  
                // inputQuad[0] = cv::Point2f(0, dst.rows*0.85);
                // inputQuad[1] = cv::Point2f(dst.cols-1, dst.rows*0.85);
                // inputQuad[2] = cv::Point2f(dst.cols-1, dst.rows-1);
          ...
          //      midean = H_result_size / 2;
           ...
           //         int area = data[cv::CC_STAT_AREA];
              ...
              //  imshow("drawing", draw_max_lable);//for visible lane max lable box
                ...
                void CalLane::fillEmptyLane(cv::Mat src_h, cv::Mat src_v, cv::Mat dst){
    
                }
                void CalLane::fixedLaneBox(cv::Mat src, cv::Mat& dst, cv::Point point1, cv::Point point2, const std::string &lane_name){
                    cv:Mat lane_box;
                    //cv::rectangle(src,cv::Point)

                    //2/3정도 roi잡아서 그 안에서 색으로 찾은거랑 소벨엣지 직선이랑 or연산하고 임계치 이하의 값은 0으로 만들기
                    //찾은 차선영역 양쪽에 박스 그리기 (같은 y축) 위치는 임의 지정 
                    //그 박스 가운데에 중앙박스 그리기
                    //중앙점을 계속 생성해서 중앙박스 안으로 가도록 
                }
}
