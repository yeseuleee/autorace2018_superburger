...
namespace lane_detect_algo
{
    ...
    class CalLane {
      public:
            ...
            struct sLine{
                double mx, my;//x 기울�?, y 기울�?
                double sx, sy;//직선?�� 중심?�� 좌표
            };
            ...
            void detectWhiteRange(cv::Mat src, cv::Mat& dst, int hmin, int hmax, int smin, int smax, int vmin, int vmax, int amin, int amax);
            ...
            int laneHist(cv::Mat src, cv::Mat dst);
            void myProjection(cv::Mat src, cv::Mat& dst, unsigned int* H_result);
            void trackingLane(cv::Mat src, cv::Mat dst);
            void makeContoursLeftLaneByHist(cv::Mat src, cv::Mat& dst);      
            void fillEmptyLane(cv::Mat src_h, cv::Mat src_v, cv::Mat dst);
            void fixedLaneBox(cv::Mat src, cv::Mat& dst, cv::Point point1, cv::Point point2, const std::string &lane_name);
